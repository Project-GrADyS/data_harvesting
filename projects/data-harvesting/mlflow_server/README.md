...
```bash
cd projects/data-harvesting/mlflow_server
cp .env.example .env
```
...
server. Run these commands from the workspace root:
...
uv run --package data-harvesting python projects/data-harvesting/scripts/evaluate.py --tracking-uri "$MLFLOW_TRACKING_URI" --run-id <MLFLOW_RUN_ID> --num-runs 10
```

You can also pass the tracking URI directly:

```bash
uv run --package data-harvesting python projects/data-harvesting/scripts/main.py --tracking-uri http://<vpn-host>:5000 -E default
uv run --package data-harvesting python projects/data-harvesting/scripts/tune.py --tracking-uri http://<vpn-host>:5000 -E tuning
```
...
`projects/data-harvesting/mlflow_server/rclone/` contains credentials and is ignored by git. The backup
