...
def test_mask_marks_only_active_agents_on_reset() -> None:
    ...
    try:
        ...
        assert not bool(sensors_mask[inactive].any())
        assert not bool(drones_mask[inactive].any())
...
def test_mask_stays_consistent_after_step() -> None:
    ...
    try:
        ...
        assert mask[active].tolist() == [True] * len(active)
        assert mask[inactive].tolist() == [False] * len(inactive)
        assert not bool(next_td.get(("agents", "observation", "sensors_mask"))[inactive].any())
        assert not bool(next_td.get(("agents", "observation", "drones_mask"))[inactive].any())
