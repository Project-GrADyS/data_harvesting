...
def test_sequential_observation_and_spec_shapes_with_agent_id(closest_sensors: int, closest_drones: int) -> None:
    ...
    try:
        ...
        assert tuple(td.get(("agents", "observation", "sensors_mask")).shape) == (num_drones, closest_sensors)
        assert tuple(td.get(("agents", "observation", "drones_mask")).shape) == (num_drones, closest_drones)
        assert td.get(("agents", "observation", "sensors_mask")).dtype == torch.bool
        assert td.get(("agents", "observation", "drones_mask")).dtype == torch.bool
        sensors_mask = td.get(("agents", "observation", "sensors_mask"))
        drones_mask = td.get(("agents", "observation", "drones_mask"))
        assert sensors_mask.sum(-1).tolist() == [min(2, closest_sensors)] * num_drones
        assert drones_mask.sum(-1).tolist() == [min(num_drones - 1, closest_drones)] * num_drones
        sensors = td.get(("agents", "observation", "sensors"))
        drones = td.get(("agents", "observation", "drones"))
        assert torch.all(sensors[~sensors_mask] == -1)
        assert torch.all(drones[~drones_mask] == -1)
        ...
        assert tuple(env.observation_spec["agents", "observation", "sensors_mask"].shape) == (num_drones, closest_sensors)
        assert tuple(env.observation_spec["agents", "observation", "drones_mask"].shape) == (num_drones, closest_drones)
...
def test_flat_observation_and_spec_shapes_with_agent_id(closest_sensors: int, closest_drones: int) -> None:
    ...
    try:
        ...
        assert tuple(td.get(("agents", "observation", "flat")).shape) == (num_drones, expected_dim)
        assert tuple(td.get(("agents", "observation", "sensors_mask")).shape) == (num_drones, closest_sensors)
        assert tuple(td.get(("agents", "observation", "drones_mask")).shape) == (num_drones, closest_drones)
