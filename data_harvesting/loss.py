...
class MaskedDDPGLoss(LossModule):
    ...
    def loss_actor(
        ...
        mask = tensordict.get(self.tensor_keys.mask, None)

        loss_actor = _reduce(loss_actor, self.reduction, mask=mask)
        self._clear_weakrefs(
            tensordict,
            loss_actor,
            "value_network_params",
    ...
    def loss_value(
        ...
        loss_value = _reduce(loss_value, self.reduction, mask=mask)
        self._clear_weakrefs(
            tensordict,
            "value_network_params",
            "target_value_network_params",
